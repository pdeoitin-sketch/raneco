...
test("Auto follows the sun: sunrise, day, sunset, night", () => {
  ...
  assert.equal(resolveAppearance({ mode: "auto", hour: 12, now: NOW, weather: sky() }).appearance, "clear");
  assert.equal(
    resolveAppearance({ mode: "auto", hour: 18, now: NOW, weather: sky({ sunset: NOW + 10 * 60000 }) }).appearance,
    "dusk",
    "sunset brings the golden hour"
  );
...
test("without weather the clock alone picks a readable look", () => {
  ...
  assert.equal(resolveAppearance({ mode: "auto", hour: 11, now: NOW, weather: null }).appearance, "clear");
  ...
  assert.equal(dayPartFromClock(12), "clear");
...
test("missing sunrise data does not invent a sunrise", () => {
  ...
  assert.equal(result.appearance, "clear");
});
...
test("only the dark palettes flip the dark UI flag", () => {
  const dark = ["dusk", "storm", "night", "dark"];
